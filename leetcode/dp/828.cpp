...
public:
    ...
    int uniqueLetterString(string s)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            if (pre1.count(s[i]))
            {
                segTree.update(1, pre1[s[i]] + 1 + 1, i + 1, 1);
                if (pre2.count(s[i]))
                    segTree.update(1, pre2[s[i]] + 1 + 1, pre1[s[i]] + 1, -1);
                else
                    segTree.update(1, 1, pre1[s[i]] + 1, -1);
            }
            else
                segTree.update(1, 1, i + 1, 1);
            sum += segTree.query(1, 1, i + 1);
            cout << sum << endl;
            if (!pre1.count(s[i]))
                pre1[s[i]] = i;
            else
