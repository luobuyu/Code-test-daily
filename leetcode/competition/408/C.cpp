...
public:
    int numberOfSubstrings(string s)
    {
        ...
        multiset<int> prefix_set;
        prefix_set.insert(0); // 插入初始前缀和
        int total_count = 0;

        for (int end = 1; end <= n; ++end)
        {
            int num_1 = prefix_1[end];
            int num_0 = prefix_0[end];

            // 查找满足条件的前缀
            for (auto it = prefix_set.begin(); it != prefix_set.end(); ++it)
            {
                int prefix_value = *it;
                if (num_1 >= (num_0 * num_0) + prefix_value)
                {
                    total_count++;
                }
                else
