// #pragma GCC optimize(2)
#include <bits/stdc++.h>
using namespace std;
#define ll long long
#define lll long long
#define PII pair<int, int>
namespace FAST_IO
{
    static string buf_line;
    static int _i;
    static int _n;
    static char _ch;
    template <class T>
    inline bool read(T &x)
    {
        T flag = 1;
        x = 0;
        if (_i >= _n)
            return false;
        _ch = buf_line[_i++];
        while (_ch < '0' || _ch > '9')
        {
            if (_ch == '-')
                flag = -1;
            _ch = buf_line[_i++];
        }
        while (_ch >= '0' && _ch <= '9')
        {
            x = (x << 3) + (x << 1) + (_ch ^ 48), _ch = buf_line[_i++];
        }
        x *= flag;
        return true;
    }

    template <class T, class... _T>
    inline bool read(T &x, _T &...y)
    {
        return read(x), read(y...);
    }

    inline bool getline()
    {
        if (!getline(cin, buf_line))
            return false;
        _i = 0, _n = buf_line.length();
        return true;
    }
    template <class T>
    inline void show(T *a, int n)
    {
        cout << "[";
        for (int i = 0; i < n; ++i)
        {
            if (i != 0)
                cout << ",";
            cout << a[i];
        }
        cout << "]";
    }

    bool endofl()
    {
        if (_i >= _n)
            return true;
        if (_i == 0)
            return false;
        if (buf_line[_i - 1] == ']')
        {
            _i++;
            return true;
        }
        return false;
    }

    template <class T, std::size_t Num>
    inline void show(T a[][Num], int n, int m)
    {
        cout << "[";
        for (int i = 0; i < n; ++i)
        {
            if (i != 0)
                cout << ",";
            show(a[i], m);
        }
        cout << "]";
    }

} // namespace FAST_IO
using namespace FAST_IO;

// int init = []
// {
//     /*********** fast_read ***************/
//     freopen("user.out", "w", stdout);
//     ios_base::sync_with_stdio(false);
//     cin.tie(nullptr);
//     cout.tie(nullptr);
//     /*************************************/

//     while (true)
//     {
//         if (!getline())
//             break;

//         getline();
//     }
//     exit(0);
//     return 0;
// }();

auto optimize_cpp_stdio = []()
{
    std::ios::sync_with_stdio(false);
    std::cin.tie(nullptr);
    std::cout.tie(nullptr);
    return 0;
}();
static const int M = 26; // 字符集大小
static const int maxn = 5e5 + 10;
static const int INF = 0x3f3f3f3f;
int nex[maxn][M], cnt; // cnt记录点号
pair<int, int> exist[maxn];
struct Trie
{
    Trie()
    {
        if (cnt == 0)
        {
            memset(exist, 0x3f, sizeof(exist));
        }
        for (int i = 0; i <= cnt; ++i)
        {
            memset(nex[i], 0, sizeof(nex[i]));
            exist[i].first = exist[i].second = INF;
        }
        cnt = 0;
    }
    void ins(string s, int index)
    {
        int now = 0, len = s.length(); // 下标从1开始
        for (int i = 0; i < len; i++)
        {
            int c = s[i] - 'a';
            if (!nex[now][c])
                nex[now][c] = ++cnt;
            now = nex[now][c];
            exist[now] = min(exist[now], {len, index});
        }
        // cout << now << endl;
    }

    int find(string s)
    {
        int now = 0, len = s.length();
        int ret = -1;
        for (int i = 0; i < len; i++)
        {
            int c = s[i] - 'a';
            if (!nex[now][c])
                return ret;
            now = nex[now][c];
            if (exist[now].second != INF)
            {
                ret = exist[now].second;
            }
        }
        return ret;
    }
};
class Solution
{
public:
    const static long long mod = 1e9 + 7;
    const long long INF_LL = 0x3f3f3f3f3f3f3f3f;
    const int INF = 0x3f3f3f3f;

    vector<int> stringIndices(vector<string> &wordsContainer, vector<string> &wordsQuery)
    {
        int n = wordsContainer.size();
        int m = wordsQuery.size();
        Trie trie;
        int minLen = INF, index;
        for (int i = 0; i < n; ++i)
        {
            string s = wordsContainer[i];
            if (s.length() < minLen)
            {
                minLen = s.length();
                index = i;
            }
            reverse(s.begin(), s.end());
            trie.ins(s, i);
        }
        vector<int> ans(m);
        for (int i = 0; i < m; ++i)
        {
            string s = wordsQuery[i];
            reverse(s.begin(), s.end());
            int ret = trie.find(s);
            if (ret == -1)
                ans[i] = index;
            else
                ans[i] = ret;
        }
        return ans;
    }
};

int t, n, m, k;
int main()
{
// #define COMP_DATA
#ifndef ONLINE_JUDGE
    freopen("in.txt", "r", stdin);
#endif
    ios::sync_with_stdio(false);
    cin.tie(0);
    Solution solution;
    vector<string> a = {"atyatzccysuonfkc", "yxnfsnwbnf", "yhqqozeuuwbmwcglqmuc", "pufhrzeeebxt", "cytfwsuyng", "gdowpewqip", "ndmrtgemimemovuqzhww", "xyxxnshfkvkoopn", "qoejmcvdxmajf", "aarvzbdlhxqzpmxmqw"};
    vector<string> b = {"wzglmxyassyocuke", "chwlfogvzsiivytadp", "dejgxaxgkftgo", "gsilmfdsgcv", "ntrdsnzihkuquwzr", "ntbdeuiqyzv", "jnkifghyandno", "cxgpkgkcfishlga", "jwhvnhhhtbhbw", "dimwutwqcfwqvytjj"};

    solution.stringIndices(a, b);
    priority_queue<int> q;
    return 0;
}